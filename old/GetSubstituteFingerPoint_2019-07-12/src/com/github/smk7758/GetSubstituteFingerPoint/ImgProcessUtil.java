...
public class ImgProcessUtil {
	...
	@Deprecated
	public static int getLargestContoursArea(List<MatOfPoint> contours) {
	...
	public static ListMap<Integer, Double> getSortedAreaNumber(List<MatOfPoint> contours) {
		// if (contours.size() < 1) throw new IllegalArgumentException("No contours.");
		double area = 0;
		ListMap<Integer, Double> contour_area = new ListMap<>();
		for (int i = 0; i < contours.size(); i++) {
			//			area = contours.get(j).size().area(); //コッチはその面積のため違う ??
			area = Imgproc.contourArea(contours.get(i)); // 普通の面積
			if (area != 0) contour_area.add(i, area);
		}

		contour_area.getInternalList().sort(Comparator.comparing(item -> -item.getValue()));
